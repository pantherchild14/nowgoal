...
// Hàm lấy lịch trình
const crawlSchedule = asyncHandler(async(date) => {
...
// update status
const updateStatusSchedule = asyncHandler(async(date) => {
    const currentTime = Date.now();
    const DOMAIN = process.env.DOMAIN;
    const replacedUrl = generateUrl(DOMAIN, date, currentTime);
    try {
...
const analysisDataTypeA = (str) => {
    ...
    const matchTime = `${arExpl[6].trim().replace(/'/g, "")}-${parseInt(
      ...
      .trim()
      .replace(/'/g, "")}`;
    const timestamp = moment(matchTime, "YYYY-M-D HH:mm:ss").valueOf();
...
module.exports = { crawlSchedule, updateStatusSchedule };